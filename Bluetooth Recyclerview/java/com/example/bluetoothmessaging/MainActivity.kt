...
class MainActivity : AppCompatActivity(), DevicesRecyclerViewAdapter.ItemClickListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bluetoothStatus = findViewById(R.id.textView4)
        bluetoothStatus?.text = getString(R.string.bluetooth_not_enabled)
        ...
        search_devices.setOnClickListener {
            val data = DeviceData("Found Devices",null)
            val list = ArrayList<DeviceData>()
            list.add(data)
            devicesAdapter.setData(list)
        ...
        devicesAdapter = DevicesRecyclerViewAdapter(context = this)
        recyclerViewPaired.adapter = devicesAdapter
        devicesAdapter.setItemClickListener(this)

        mReceiver = DeviceReceiver(mDeviceList = mDeviceList,devicesAdapter = devicesAdapter,progressBar = progressBar)
        ...
        if (mBtAdapter == null)
            ...
            if (pairedDevices?.size ?: 0 > 0) {
                mPairedDeviceList.add(DeviceData("Paired Devices",null))
            ...
            devicesAdapter.setData(mPairedDeviceList)
        }
    ...
    private fun findAllActiveDevices() {
...
//                mPairedDeviceList.add(DeviceData("Found Devices",null))
//                if (pairedDevices?.size ?: 0 > 0) {
//                    for (device in pairedDevices!!) {
//                        val deviceName = device.name
//                        val deviceHardwareAddress = device.address
...
//                }
                ...
                bluetoothStatus?.text = getString(R.string.not_connected)
            }
    ...
    override fun itemClicked(deviceData: DeviceData) {
        connectedAllDeviceShow(deviceData)
    }

    private fun connectedAllDeviceShow(deviceData: DeviceData) {
        ...
        bluetoothStatus?.text = getString(R.string.connecting)
    ...
    object : Handler() {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                Components.MESSAGE_STATE_CHANGE -> {
                ...
                Components.MESSAGE_WRITE -> {
                    val writeBuf = msg.obj as ByteArray
                    val writeMessage = String(writeBuf)
                    //Toast.makeText(this@MainActivity,"Me: $writeMessage",Toast.LENGTH_SHORT).show()
                    //mConversationArrayAdapter.add("Me:  " + writeMessage)
                    val milliSecondsTime = System.currentTimeMillis()
                    chatFragment.communicate(com.webianks.bluechat.Message(writeMessage,milliSecondsTime,Components.MESSAGE_TYPE_SENT))

                }
                Components.MESSAGE_READ -> {
                    val readBuf = msg.obj as ByteArray
                    val readMessage = String(readBuf, 0, msg.arg1)
                    val milliSecondsTime = System.currentTimeMillis()
                    //Toast.makeText(this@MainActivity,"$mConnectedDeviceName : $readMessage",Toast.LENGTH_SHORT).show()
                    //mConversationArrayAdapter.add(mConnectedDeviceName + ":  " + readMessage)
                    chatFragment.communicate(com.webianks.bluechat.Message(readMessage,milliSecondsTime,Components.MESSAGE_TYPE_RECEIVED))
                }
                Components.MESSAGE_DEVICE_NAME -> {
                    mConnectedDeviceName = msg.data.getString(Components.DEVICE_NAME).toString()
                    bluetoothStatus?.text = getString(R.string.connected_to) + " " +mConnectedDeviceName
//                    connectionDot.setImageDrawable(getDrawable(R.drawable.ic_circle_connected))
                    //Snackbar.make(findViewById(R.id.mainScreen),"Connected to " + mConnectedDeviceName,Snackbar.LENGTH_SHORT).show()
                    connected = true
                    showChatFragment()
                }
                Components.MESSAGE_TOAST -> {
                    bluetoothStatus?.text = getString(R.string.not_connected)
//                    connectionDot.setImageDrawable(getDrawable(R.drawable.ic_circle_red))
                    //Snackbar.make(findViewById(R.id.mainScreen),msg.data.getString(Components.TOAST).toString(),Snackbar.LENGTH_SHORT).show()
                    connected = false
                  }
            }
        }
