...
class BluetoothChatService(context: Context, handler: Handler){
    ...
    @Synchronized fun connected(socket: BluetoothSocket?, device: BluetoothDevice?, socketType: String) {
        ...
        val msg = mHandler?.obtainMessage(Components.MESSAGE_DEVICE_NAME)
        val bundle = Bundle()
        bundle.putString(Components.DEVICE_NAME, device?.name)
        msg?.data = bundle
    ...
    private fun connectionFailed() {
        val msg = mHandler?.obtainMessage(Components.MESSAGE_TOAST)
        val bundle = Bundle()
        bundle.putString(Components.TOAST, "Unable to connect device")
        msg?.data = bundle
        mHandler?.sendMessage(msg)
    ...
    private fun connectionLost() {
        ...
        val msg = mHandler?.obtainMessage(Components.MESSAGE_TOAST)
        val bundle = Bundle()
        bundle.putString(Components.TOAST, "Device connection was lost")
        msg?.data = bundle
        mHandler?.sendMessage(msg)
    ...
    private inner class ConnectedThread(private val mmSocket: BluetoothSocket?, socketType: String) : Thread() {
        ...
        override fun run() {
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    ...
                    mHandler?.obtainMessage(Components.MESSAGE_READ, bytes, -1, buffer)
                            ?.sendToTarget()
                } catch (e: IOException) {
        ...
        fun write(buffer: ByteArray) {
            try {
                ...
                mHandler?.obtainMessage(Components.MESSAGE_WRITE, -1, -1, buffer)
                        ?.sendToTarget()
            } catch (e: IOException) {
